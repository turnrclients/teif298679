...
function urlToFile(url, filename, callback) {
  fetch(url)
    .then(r => r.blob())
    .then(b => callback(new File([b], filename, { type: b.type })))
    .catch(() => alert("URL image cannot be imported. Use Upload."));
}
...
function uploadImgData(file, originalEl) {
  ...
  formData.set('imgFileName', getFileNameFromImgSrc(originalEl));
  formData.set('clientName', clientName);
  formData.set('clientProjectName', clientProjectName);
...
$(document).on('click', '.updateImg', function (e) {
  ...
  e.preventDefault();
openImagePicker(this.tagName === 'IMG' ? this : $(this).find('img')[0]);
});
