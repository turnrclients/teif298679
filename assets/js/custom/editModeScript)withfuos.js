...
// for Upload images
function uploadViaFuos(formData, originalEl) {
  $.ajax({
    type: "POST",
    ...
    cache: false,
    success: function () {
      var imgName = $(originalEl).attr("src");
      if (imgName.includes("?")) {
        imgName = imgName.split("?")[0];
      }

      $(originalEl).attr("src", imgName + "?" + new Date().getTime());
    },
    error: function (xhr) {
...
$('#confirmImage').click(function () {
  ...
  // FILE SELECTED (UPLOAD TAB)
  if (selectedFile) {
    formData.append("file", selectedFile);
    uploadViaFuos(formData, el);
  }
  // URL / ASSETS / PEXELS
